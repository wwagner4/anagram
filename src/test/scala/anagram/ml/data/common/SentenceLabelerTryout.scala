...
object SentenceLabelerTryout extends App {
  ...
  val labeled = labeler.labelSentence(sentences)

  for ((r, i) <- labeled.toSeq.sortBy(-_.label).zipWithIndex) {
